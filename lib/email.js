      ...
      body: JSON.stringify({
        ...
        ...(adjuntos.length ? {
          attachments: adjuntos.map(a => ({
            ...
            /* El QR del cuerpo va embebido: el HTML lo llama por `cid:`, así
               que Resend necesita saber su `content_id` y que es `inline`. Sin
               estos dos campos lo mandaría como un archivo colgando al final y
               el hueco del QR quedaría vacío — que es justo el fallo que se
               vino a quitar al dejar de pedirle el dibujo a un tercero.
               Los adjuntos normales (el .ics) no traen `cid` y siguen igual. */
            ...(a.cid ? { content_id: a.cid, content_disposition: 'inline' } : {}),
          })),
        } : {}),
      }),
    });
