 ...
 *
 * Se mira el CÓDIGO y no el texto porque el texto cambia con el idioma del
 * servidor, y una comparación de cadenas que falla aquí convierte un duplicado
 * legítimo en un error 500 — con la persona delante y la fila esperando.
 *
 * Los dos motores, porque el corte a servidor propio ya está en marcha:
 *
 *   Postgres  23505 · unique_violation
 *   MySQL     1062  · ER_DUP_ENTRY
 *
 * El día del corte esto es lo primero que se rompería en silencio: en MySQL el
 * choque contra el índice llega con otro código y otro texto —«Duplicate
 * entry», que ni siquiera contiene «duplicate key»—, así que se contestaría
 * 500 en vez de «ya lo recibió» y quien entrega acabaría dando el almuerzo dos
 * veces «porque la máquina dio error».
 *
 * Se acepta también el texto como último recurso: un conector que envuelva el
 * error y pierda el código sigue diciéndolo en el mensaje. */
const CODIGOS_DUPLICADO = new Set(['23505', '1062', 'ER_DUP_ENTRY']);

function esDuplicado(error) {
  if (!error) return false;
  if (CODIGOS_DUPLICADO.has(String(error.code))) return true;
  if (CODIGOS_DUPLICADO.has(String(error.errno))) return true;
  return /duplicate key|duplicate entry|unique constraint/i.test(String(error.message || ''));
}
...
module.exports = {
  ...
  siguienteUso, esDuplicado, CODIGOS_DUPLICADO,
