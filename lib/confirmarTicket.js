...
async function confirmarTicketPagado(ticketId, monto, via) {
  ...
  if (tFull?.ticket_type_id) {
    ...
    /* Suma en la base y no aquí: dos compras a la vez leían el mismo
       `vendidos` y escribían el mismo número, y una se perdía. Ver 0138. */
    if (tt) await supabase.rpc('sumar_vendidos_tipo', { p_tipo: tFull.ticket_type_id, p_delta: 1 });
  }
  ...
  if (evWh?.owner_id) {
    ...
    /* Y por lo mismo: `x = x + n` DENTRO de la base toma el cerrojo de la fila,
       así que dos ventas simultáneas se ponen en fila solas. Leyendo y
       escribiendo desde aquí, FESTECH IBAGUÉ perdió 212 personas. Ver 0138. */
