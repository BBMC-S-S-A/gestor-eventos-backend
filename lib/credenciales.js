...
function fichaDeLaPuerta({ puesto, ticket, enlace = null }) {
  return {
    nombre    : puesto?.nombre || ticket?.guest_nombre || null,
    documento : puesto?.documento || null,
    foto_url  : fotoParaVer(puesto?.foto_url, enlace),
...
/* ── Quién sigue dentro ─────────────────────────────────────────────────
 *
 * El día del montaje esto vale más que la identificación. Saber a quién se
 * dejó entrar no dice nada a las ocho de la noche; saber **quién no ha
 * salido** es lo que hace que alguien vaya a mirar antes de cerrar el galpón.
 *
 * El vaivén ya se lleva por persona desde la 0125 (`ticket_movimientos.puesto_id`).
 * Lo único que faltaba era leerlo: por cada persona, su ÚLTIMO movimiento. Si
 * fue una entrada, sigue dentro.
 *
 * Los movimientos llegan del más nuevo al más viejo, así que el primero que se
 * ve de cada puesto es el que manda y el resto se ignora. Hacerlo al revés
 * —quedarse con el último visto— daría el primer movimiento del día, que es
 * siempre una entrada: todo el mundo aparecería dentro para siempre. */
function quienSigueDentro(movimientos = []) {
  const visto = new Set();
  const dentro = new Map();

  for (const m of movimientos) {
    if (!m?.puesto_id || visto.has(m.puesto_id)) continue;
    visto.add(m.puesto_id);
    if (m.tipo === 'entrada') dentro.set(m.puesto_id, m.created_at || null);
  }
  return dentro;
}
 ...
 *
 * Lo guardado puede ser dos cosas y las dos son legítimas:
 *
 *   · una RUTA del almacén privado, que es lo que sube la cuadrilla desde el
 *     enlace del stand. Es la cara de un trabajador junto a su documento: dato
 *     personal, así que vive en una carpeta privada y sólo se lee con un
 *     enlace firmado de quince minutos;
 *   · una URL entera, para lo que venga de fuera o de antes.
 *
 * Se distinguen por el `://` y no por una columna más: una columna que hay que
 * mantener de acuerdo con el contenido de otra acaba desacordada.
 *
 * Sin firmador —cuando quien pregunta no puede ver fotos— se devuelve `null` y
 * no la ruta: una ruta suelta en una respuesta es media filtración. */
function fotoParaVer(valor, enlace) {
  if (!valor) return null;
  if (/^https?:\/\//i.test(valor)) return valor;
  return typeof enlace === 'function' ? enlace(valor) : null;
}
...
module.exports = {
  ...
  puedeAbrir, fichaDeLaPuerta, fotoParaVer, quienSigueDentro,
};
