...
const COLUMNAS = 'id, ticket_id, evento_id, orden, nombre, email, documento, estado, qr_token, credencial_gen, usado_at, autorizado_at, autorizado_por, foto_url, telefono';
 ...
 * la GENERACIÓN siga siendo la que trae el QR. Si entre la lectura y la
 * escritura alguien transfirió el puesto, la generación subió y esta escritura
 * ya no encuentra fila — que es lo correcto: la credencial que se está
 * presentando acaba de dejar de valer.
 *
 * Era el `qr_token` y ahora es la generación (0127), y no es lo mismo: el
 * token cambia también al reenviar el correo, y con la comparación anterior
 * ese reenvío tumbaba una credencial que nadie había transferido. */
async function marcarPuestoUsado({ puesto, gen = 0, at }) {
  const { data, error } = await supabase
    .from('ticket_puestos')
    .update({ estado: 'usado', usado_at: at })
    .eq('id', puesto.id)
    .eq('credencial_gen', Number(gen) || 0)
    .neq('estado', 'usado')
    .select(COLUMNAS);
  ...
  if (!data || data.length === 0) {
    ...
    return { ok: false, ...veredictoDePuesto({ puesto: ahora, gen }), puesto: ahora };
  }
 ...
 */
async function consumirPuesto({ ticket, puestoIdDelQr, token, gen = 0, at, puestos: yaLeidos = null }) {
  ...
  if (puestoIdDelQr) {
    ...
    const v = veredictoDePuesto({ puesto, token, gen, ticketId: ticket.id });
  ...
  /* La generación con la que se cierra es la del PUESTO y no la del QR: en el
     camino de arriba ya se comprobó que son la misma, y en el de abajo —el QR
     de la boleta en una mesa— no hay generación que traer. */
  const marcado = elegido.qr_token
    ? await marcarPuestoUsado({ puesto: elegido, gen: elegido.credencial_gen || 0, at })
