...
/* ¿Abre la puerta este QR de puesto?
 *
 * ── Por qué no basta con que la firma sea válida ─────────────────────────
 *
 * `verifyTicketQR` dice que el token lo firmamos nosotros y no está tocado.
 * Eso es todo lo que puede decir: es criptografía, no memoria. Y como los QR
 * de boleta no caducan —valen mientras el evento esté activo—, un token viejo
 * sigue pasando esa comprobación para siempre.
 *
 * Para un puesto eso no vale, porque su credencial ROTA. La 0118 apuesta a
 * esto para la reventa: al transferir, el puesto se firma de nuevo y «el
 * anterior deja de servir por construcción». Pero por construcción no deja de
 * servir nada: el token viejo sigue teniendo una firma nuestra perfectamente
 * buena. Lo único que lo distingue del nuevo es que ya no es el que guarda la
 * base, así que hay que MIRAR la base — y compararlo.
 *
 * Sin esta comparación, quien vendió su puesto se queda con un QR que abre la
 * puerta igual que el de quien se lo compró. Dos personas, una silla, y el
 * escáner dejando pasar a las dos: exactamente la estafa que la transferencia
 * venía a hacer imposible.
 *
 * `token` es lo que acaba de leer la cámara; `puesto` es la fila de la base.
 */
const PUERTA_CERRADA = {
  no_existe:  'Este QR apunta a un puesto que ya no existe.',
  otra_boleta:'Este QR no es de esta boleta.',
  rotado:     'Este QR ya no sirve: el puesto se transfirió a otra persona y tiene un código nuevo.',
  sin_token:  'Este puesto está a medio transferir y todavía no tiene credencial. Quien organiza puede volver a emitirla.',
  usado:      'Este puesto ya entró al evento.',
};

function veredictoDePuesto({ puesto, token, ticketId = null, gen = 0 }) {
  const no = (motivo) => ({ ok: false, motivo, mensaje: PUERTA_CERRADA[motivo] });

  if (!puesto) return no('no_existe');
  /* El token dice a qué boleta pertenece; si no cuadra con el puesto que la
     base tiene con ese id, algo se armó a mano. */
  if (ticketId && puesto.ticket_id && puesto.ticket_id !== ticketId) return no('otra_boleta');
  /* Un puesto a medio transferir: `aplicarTransferencia` deja `qr_token` en
     null a propósito, para que un fallo a mitad no deje dos credenciales
     buenas. Aquí eso es «todavía no abre», no «pasa». */
  if (!puesto.qr_token) return no('sin_token');
  /* Lo que invalida NO es que el token sea otro: es que sea de una generación
     anterior (0127).
     Comparar contra el token guardado parecía lo mismo y no lo era. Una boleta
     se reenvía —el correo se pierde y la persona lo pide otra vez—, y cada
     envío firma un token nuevo del mismo puesto: con la comparación de
     cadenas, quien llegaba con el primer correo se encontraba la puerta
     cerrada y un mensaje que decía «se transfirió», que además era mentira.
     La generación sube SÓLO al transferir, así que los reenvíos abren todos y
     la reventa sigue matando al anterior, que es lo que se quería de las dos. */
  if ((Number(puesto.credencial_gen) || 0) !== (Number(gen) || 0)) return no('rotado');
...
/* A quién le toca este escaneo de reingreso, y en qué sentido.
 *
 * ── Por qué alternar no vale en una mesa ─────────────────────────────────
 *
 * El reingreso sin `tipo` alterna según el último movimiento de la BOLETA:
 * si la última fue entrada, ésta es salida. Con una persona por boleta eso es
 * justo lo que se quiere. Con cuatro compartiendo boleta, el escáner alterna
 * entre ellos — entra la 1, «sale» la 2, entra la 3, «sale» la 4 — y el aforo
 * de la zona acaba diciendo que no hay nadie mientras entran cuatro personas.
 * Nadie ve un error: un número tranquilo y equivocado en el tablero que decide
 * si se cierra una puerta.
 *
 * Con puestos, cada persona lleva su propio ir y venir. Cuando el QR dice cuál
 * es (`pid`), se alterna el de ESE puesto. Cuando es el QR de la boleta —una
 * sola credencial para toda la mesa—, no hay forma de saber quién de los
 * cuatro está enseñándolo, así que se elige por el sentido: al entrar, alguien
 * que esté fuera; al salir, alguien que esté dentro. No identifica a la
 * persona, pero la CUENTA es correcta, que es para lo que existe el aforo.
 *
 * `dentro` son los ids de los puestos que ahora mismo están dentro.
 */
const VAIVEN_IMPOSIBLE = {
  todos_dentro: 'Ya entraron todas las personas de esta boleta.',
  nadie_dentro: 'No hay nadie de esta boleta dentro del evento.',
};

function vaivenDeLaMesa({ puestos = [], dentro = [], tipoPedido = null }) {
  const estanDentro = new Set(dentro);
  const fuera = puestos.filter(p => !estanDentro.has(p.id));
  const adentro = puestos.filter(p => estanDentro.has(p.id));

  /* Sin sentido pedido: si queda alguien por entrar, entra; si están todos
     dentro, el escaneo sólo puede querer decir que alguien se va. */
  const tipo = (tipoPedido === 'entrada' || tipoPedido === 'salida')
    ? tipoPedido
    : (fuera.length > 0 ? 'entrada' : 'salida');

  const candidatos = tipo === 'entrada' ? fuera : adentro;
  if (candidatos.length === 0) {
    const motivo = tipo === 'entrada' ? 'todos_dentro' : 'nadie_dentro';
    return { ok: false, motivo, mensaje: VAIVEN_IMPOSIBLE[motivo], tipo };
  }

  /* El de menor `orden`, para que al menos sea estable y explicable: «salió el
     puesto 2» en vez de uno al azar cada vez. */
  const elegido = [...candidatos].sort((a, b) => (a.orden || 0) - (b.orden || 0))[0];
  return { ok: true, tipo, puesto: elegido };
}
...
function aplicarTransferencia({ puesto, destino, monto = null, currency = null, via = 'titular', actorId = null }) {
  return {
    puesto: {
      ...
      qr_token: null,
      /* Sube la generación (0127). Esto —y no el hecho de que el token cambie—
         es lo que mata a la credencial anterior: un token distinto lo produce
         también un reenvío del correo, y ése no invalida nada. Sin este número,
         la única forma de invalidar era comparar el token guardado, que se
         llevaba por delante los reenvíos legítimos. */
      credencial_gen: (Number(puesto.credencial_gen) || 0) + 1,
      estado: 'asignado',
      asignado_at: new Date().toISOString(),
      /* Quien recibe un puesto transferido no hereda la autorización de quien
         se lo pasó: si el tipo la exige, hay que volver a responder por la
         persona nueva. Es justo lo que se compra con una reventa de una
         credencial de montaje. */
      autorizado_at: null,
      autorizado_por: null,
    },
    rastro: {
...
module.exports = {
  ...
  veredictoDePuesto, PUERTA_CERRADA,
  vaivenDeLaMesa, VAIVEN_IMPOSIBLE,
