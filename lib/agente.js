...
/* Groq contesta 404 cuando el MODELO no existe: retira modelos cada pocos
   meses, y un GROQ_MODEL que funcionaba deja de hacerlo sin que cambie nada de
   este lado. Con Groq como único proveedor configurado, «Crear con IA» y el
   chat morían con «groq 404». Ante un 404 se prueba el siguiente modelo de la
   lista; cualquier otro error sale tal cual. El modelo que funcionó se queda
   para las siguientes llamadas. */
const GROQ_RESPALDOS = [...new Set([GROQ_MODEL, 'llama-3.3-70b-versatile', 'llama-3.1-8b-instant'])];
let groqModeloVivo = GROQ_MODEL;

async function pedirGroq(cuerpo) {
  const orden = [groqModeloVivo, ...GROQ_RESPALDOS.filter(m => m !== groqModeloVivo)];
  let res;
  for (const model of orden) {
    ...
    if (res.status !== 404) {
      if (res.ok && model !== groqModeloVivo) {
        console.warn(`[agente] Groq: el modelo ${groqModeloVivo} no existe; se usa ${model}. Revisa GROQ_MODEL.`);
        groqModeloVivo = model;
      }
      break;
    }
  }
...
async function chatGroq(userId, msgs) {
  ...
  for (let guard = 0; guard < GUARD_MAX; guard++) {
    const data = await pedirGroq({
      messages, tools, tool_choice: 'auto', temperature: 0.3, max_tokens: 1500,
    });
  ...
  for (const p of PROVIDERS) {
    try {
      ...
      if (p === 'anthropic') {
        ...
        const j = await pedirGroq({ max_tokens: maxTokens, messages: [{ role: 'user', content: prompt }] });
