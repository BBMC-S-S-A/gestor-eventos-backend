...
const { qrParaCorreo } = require('./qrImagen.js');
 ...
 *
 * Devuelve `{ html, adjuntos }`: el QR es un PNG que se genera aquí y viaja
 * embebido en el mensaje, no una URL. Ver `lib/qrImagen.js` — el token es la
 * credencial de entrada y pedirle el dibujo a un servicio ajeno lo dejaba
 * escrito en los logs de acceso de un tercero. */
function bloqueDatos({ marca, ctx, conQr, qrEnLinea = false }) {
  const filas = [];
  ...
  /* El mismo valor que enseña la boleta en pantalla y que imprime el PDF:
     `qr_token` y, si todavía no está firmado, el código corto — que la puerta
     acepta igual (`lib/ticketLookup.js`). */
  const valorQr = conQr ? (ctx.qr_token || ctx.codigo || '') : '';
  const imagenQr = valorQr ? qrParaCorreo(valorQr) : null;

  /* En la vista previa del panel no hay mensaje donde embeber nada: el HTML se
     pinta en un navegador, y ahí un `cid:` es una imagen rota. Se mete el
     mismo PNG como `data:`, que el navegador sí dibuja. Al revés no vale —en
     Gmail las imágenes `data:` están bloqueadas—, y por eso el correo de
     verdad sigue yendo por `cid:`. */
  const adjuntos = (imagenQr && !qrEnLinea) ? [imagenQr.adjunto] : [];
  const fuenteQr = imagenQr
    ? (qrEnLinea
        ? `data:image/png;base64,${imagenQr.adjunto.content.toString('base64')}`
        : `cid:${imagenQr.cid}`)
    : null;
  ...
  if (filas.length === 0 && !ctx.codigo && !qr) return { html: '', adjuntos: [] };
...
function renderEmail({ tipo, plantilla = {}, evento = {}, ctx = {}, qrEnLinea = false }) {
  ...
  /* El QR va embebido en el mensaje, así que el bloque de datos devuelve
     también lo que hay que adjuntar. */
  const datos = bloqueDatos({ marca, ctx, conQr: def.conQr === true, qrEnLinea });

  const html = cascaron({
    marca,
    encabezado,
    cuerpoHtml,
    extraHtml: datos.html,
    botonTexto,
    botonUrl,
  ...
  /* `adjuntos` son los del propio cuerpo —hoy el PNG del QR—. El .ics lo pone
     `enviarEmailEvento`, que es quien sabe si este tipo lleva calendario. */
  return { asunto, html, adjuntos: datos.adjuntos };
}
...
async function enviarEmailEvento({ evento: eventoEntrada, tipo, to, ctx = {}, registrar = true,
  ...
  try {
    ...
    const { asunto, html, adjuntos: adjuntosCuerpo } = renderEmail({ tipo, plantilla, evento, ctx: contexto });
    asuntoFinal = asunto;
    ...
    if (buzonPropio?.agotados) {
      ...
      const envio = await sendMail({
        ...
        /* El QR embebido del cuerpo y, si el tipo lo lleva, el .ics. */
        attachments: [...(adjuntosCuerpo || []), ...cal.adjuntos],
        smtp: buzonPropio,
      });
      resultado = envio?.ok
