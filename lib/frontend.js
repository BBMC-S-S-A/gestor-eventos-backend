...
/* El respaldo cuando no hay ni `FRONTEND_URL` ni `CORS_ORIGINS`.
 *
 * Era `https://gestor-eventos-frontend.vercel.app`, y Vercel se retiró del todo
 * el 17-sep: ese dominio ya no es nuestro. Un valor por defecto que apunta a un
 * sitio que no controlamos es peor que no tener ninguno — los enlaces de los
 * correos son lo unico que la persona guarda y reenvia, y habrian ido a parar a
 * donde quiera que acabe ese subdominio.
 *
 * Hoy el panel vive en cPanel, en el dominio de abajo. Sigue siendo un respaldo
 * y nada mas: quien necesita saber si la variable esta puesta de verdad pregunta
 * por `frontendDeclarado()`, que es lo que ya hacen `lib/oauth.js` y
 * `lib/emailPlantillas.js` para no prometer una direccion de fabrica. */
const PORDEFECTO = 'https://gestekeventost.dpdns.org';
